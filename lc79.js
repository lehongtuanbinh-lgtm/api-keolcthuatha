    ...
    predict(history) {
        ...
        for (const a of this.algs) {
            try {
                ...
                if (a.id === 'algo15_aivilong_multiinversion') weight *= 2.5; // Vi Long siêu trọng số cho 1-1, 2-2
...
app.get("/api/taixiumd5/lc79", async () => {
    ...
    return {
        ...
        xuc_sac: lastResult.dice, // Đã bổ sung trả về mảng xúc xắc
        pattern: pattern,
...
app.get("/", async () => { 
    return {
        status: "ok",
        msg: "AI Tài Xỉu MD5 Pro - Phiên bản Pattern Master Ultimate VIP VI LONG",
        version: "7.1 AI Vi Long Multi Inversion", 
        algorithms: ALL_ALGS.length,
        pattern_recognition: "VIP Hoàn Chỉnh (Bám bệt cực chặt, Khóa 1-1 & 2-2 tối thượng, AI tự học, Nhận diện chuỗi chìm)",
        endpoints: [
            "/api/taixiumd5/lc79",
...
const start = async () => {
    ...
    console.log("\n🚀 AI Tài Xỉu MD5 Pro V7.1 - Ultimate VIP VI LONG Đã Khởi Động!");
    ...
    console.log("\n🎯 CẬP NHẬT GIA TRỌNG MỚI (V7.1 AI VI LONG):");
    console.log("   • [THUẬT TOÁN 14]: Nâng cấp cơ chế phân tích dày đặc Fuzzy Logic để bắt mảng chìm.");
    console.log("   • [THUẬT TOÁN 15]: Tích hợp AI tự học Đa đảo chiều thuật toán, Momentum Scanner.");
    console.log("   • Bắt cứng ngắc 1-1, 2-2 không trượt nhịp, không bẻ lệch kết quả.");
    console.log("   • Cập nhật JSON Output đầy đủ ID, Xúc xắc, No Random theo chuẩn @cskhvilong.");
};
start();